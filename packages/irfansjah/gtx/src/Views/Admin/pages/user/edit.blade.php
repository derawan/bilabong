        ...
        <div class="box-body">
            ...
            <form method="POST" action="{{ route('admin.users.update') }}" enctype="multipart/form-data">
                ...
                <div class="row">
                        ...
                        <div class="form-group has-feedback @error('avatar') has-error @enderror">
                            ...
                            <select id="role" name="roles" class="form-control">
                                @foreach($roles as $role)
                                    <option value="{{$role->name}}" {{ in_array($role->name,collect($user->roles()->pluck('name'))->all())?"selected":""  }}>{{$role->name}}</option>
                                @endforeach
                            </select>
                  ...
                  <div class="row">
                    <div class="col-xs-3">
                    <a href="{{route('admin.users')}}" class="btn btn-success btn-block btn-flat">{{ __('Kembali') }}</a>
                      </div>
                    <div class="col-xs-6">
                    </div>
                    <!-- /.col -->
