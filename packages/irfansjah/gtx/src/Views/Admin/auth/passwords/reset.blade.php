...
<div class="col-md-12" style="margin-top:20px">
    <form method="POST" action="{{ $resetpass_route }}">
        @csrf
        <input type="hidden" name="token" value="{{ $token }}">
    <p class="login-box-msg">{{ __('Reset Password') }}</p>
        <div class="form-group has-feedback @error('email') has-error @enderror">
          <input type="email" id="email" class="form-control" name="email" value="{{ $email ?? old('email') }}" required autocomplete="off" autofocus placeholder="{{ __('E-Mail Address') }}">
        ...
        <div class="row">
          ...
          <div class="col-xs-8"><button type="submit" class="btn btn-primary btn-block btn-flat"><i class="glyphicon glyphicon-log-in"></i>  &nbsp; {{ __('Reset Password') }}</button></div>
